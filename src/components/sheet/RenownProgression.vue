...
<template>
  <section class="bg-surface-container-highest rounded-lg p-6 border border-outline-variant shadow-sm w-full">
    <div class="flex justify-between items-center border-b border-primary-container pb-3 mb-6">
      <h3 class="font-headline-md text-headline-md text-primary flex items-center gap-2">
        <span class="material-symbols-outlined">military_tech</span> Renown & Progression
      </h3>
    ...
    <div class="max-w-2xl mx-auto w-full">
      <div class="flex justify-between items-center mb-4">
        <span class="font-label-md text-label-md text-on-surface-variant uppercase">Great Works Completed</span>
        <span v-if="getMilestoneData().tier < 4" class="font-headline-md text-headline-md text-tertiary">
      ...
      <div v-if="getMilestoneData().tier < 4" class="flex gap-4 mb-4">
        <div 
          ...
          :class="[
            (store.currentCharacterData.milestones || 0) >= i ? 'bg-tertiary border border-tertiary shadow-[0_0_10px_rgba(234,194,73,0.3)]' : 'bg-surface-container border border-outline-variant hover:border-tertiary/50'
          ]"
        >
          <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <span class="text-[8px] uppercase font-bold" :class="(store.currentCharacterData.milestones || 0) >= i ? 'text-on-tertiary' : 'text-outline'">
              Milestone {{ i }}
            </span>
          </div>
        </div>
