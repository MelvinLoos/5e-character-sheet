...
.stat-orb {
  background: linear-gradient(145deg, #37352b, #1e1c13);
  border: 1px solid theme('colors.tertiary-container');
}
</style>
...
<template>
  <div class="flex flex-col gap-8 w-full">
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-card-gap lg:grid-cols-3">
      <!-- Gold -->
      <div
        class="bg-surface-container p-4 rounded border border-[#1A3C40] flex items-center justify-between"
      >
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 stat-orb rounded-full flex items-center justify-center text-tertiary"
          >
            <span
          ...
          <div>
            ...
            <input
              ...
              type="number"
              v-model="store.currentCharacterData.gold"
              class="w-20 bg-background border border-[#1A3C40] rounded text-on-surface p-1 text-headline-lg font-headline-lg"
            />
            <span v-else class="font-headline-lg text-headline-lg text-tertiary">{{
              store.currentCharacterData.gold
            }}</span>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-full flex items-center justify-center bg-secondary-container text-secondary border border-secondary"
          >
            <span
              class="material-symbols-outlined text-2xl"
              style="font-variation-settings: 'FILL' 1"
          ...
          <div>
            ...
            <input
              v-if="store.isEditing"
              type="number"
              v-model="store.currentCharacterData.supply"
              class="w-20 bg-background border border-[#1A3C40] rounded text-on-surface p-1 text-headline-lg font-headline-lg"
            />
            <span v-else class="font-headline-lg text-headline-lg text-secondary">{{
              store.currentCharacterData.supply
            }}</span>
          </div>
        </div>
        <button
      ...
      >
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-full flex items-center justify-center bg-primary-container text-primary border border-primary"
          >
            <span
              class="material-symbols-outlined text-2xl"
              style="font-variation-settings: 'FILL' 1"
          ...
          <div>
            ...
            <input
              v-if="store.isEditing"
              type="number"
              v-model="store.currentCharacterData.influence"
              class="w-20 bg-background border border-[#1A3C40] rounded text-on-surface p-1 text-headline-lg font-headline-lg"
            />
            <span v-else class="font-headline-lg text-headline-lg text-primary">{{
              store.currentCharacterData.influence
            }}</span>
          </div>
        </div>
      </div>
    ...
    <section>
      ...
      <div
        class="h-4 bg-surface-container-highest rounded-full overflow-hidden border border-[#1A3C40] flex"
      >
        <div
          class="h-full bg-tertiary transition-all duration-300"
    ...
    <section>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-card-gap">
        <div
          v-for="(gear, index) in store.currentCharacterData.equippedGear"
          :key="gear.id"
          :class="[
            'p-5 rounded border border-[#1A3C40] relative shadow-md transition-colors',
            getGearBgClass(gear.theme),
          ]"
        >
          <div v-if="store.isEditing" class="flex flex-col gap-2 mb-2">
            ...
            <div class="flex gap-2 text-sm">
              ...
              />
              <input
                v-model.number="gear.slotCost"
                type="number"
                placeholder="Cost"
                class="w-16 bg-black/20 border border-black/30 rounded p-1 text-inherit"
              />
            </div>
            <div class="flex gap-2 text-sm">
              <select
            ...
            <textarea
              ...
              class="w-full bg-black/20 border border-black/30 rounded p-1 text-inherit text-sm min-h-[60px]"
            ></textarea>
          </div>
          <template v-else>
            <div
              :class="[
                'absolute top-0 right-0 font-label-md text-label-md px-3 py-1 rounded-bl border-l border-b border-[#1A3C40]',
                gear.theme === 'parchment'
                  ? 'bg-[#15130b] text-[#e8e2d4]'
                  : gear.theme === 'deep-teal'
                    ? 'bg-[#15130b] text-primary'
            ...
            <div class="flex items-start gap-4 mb-3">
              ...
              <div>
                <h4 class="font-headline-md text-headline-md leading-tight">
                  {{ gear.name || 'Unnamed Item' }}
                </h4>
                <span class="font-label-md text-label-md opacity-80">{{
                  gear.type || 'Gear'
                }}</span>
              </div>
            </div>
      ...
      <div
        v-if="!store.currentCharacterData.equippedGear?.length"
        class="text-on-surface-variant text-center py-8 italic border border-dashed border-outline-variant rounded"
      >
        No gear.
      </div>
    </section>
    ...
    <section>
      ...
      <div class="bg-surface-container rounded border border-[#1A3C40] overflow-hidden">
        ...
        <div
          v-for="(consumable, index) in store.currentCharacterData.consumables"
          :key="consumable.id"
          class="grid grid-cols-12 gap-4 p-4 border-b border-surface-variant items-center hover:bg-surface-container-high"
        >
          <div class="col-span-6 flex items-center gap-3">
            ...
            <div class="w-full">
              ...
              <template v-else>
                <div class="font-body-lg">{{ consumable.name || 'Unnamed' }}</div>
          ...
          <div class="col-span-3 text-center">
            <input
              v-if="store.isEditing"
              v-model.number="consumable.slotCost"
              type="number"
    ...
    <section>
      <h3
        class="font-headline-md text-headline-md text-on-surface mb-4 border-b border-[#1A3C40] pb-2 flex items-center gap-2"
      >
      ...
      <div class="bg-surface-container p-5 rounded-2xl border border-[#1A3C40] shadow-md relative">
        <textarea
          v-if="store.isEditing"
          v-model="store.currentCharacterData.equipment"
          class="w-full bg-background border border-[#1A3C40] rounded-xl p-3 text-on-surface font-body-md min-h-[150px]"
        ></textarea>
        <div
          v-else
          class="font-body-md text-body-md text-on-surface-variant whitespace-pre-wrap leading-relaxed"
        >
          {{ store.currentCharacterData.equipment || 'No equipment listed.' }}
        </div>
      </div>
    </section>
  </div>
