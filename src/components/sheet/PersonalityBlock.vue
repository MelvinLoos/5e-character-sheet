...
<template>
  <section
    class="bg-surface-container-high rounded-lg border border-outline-variant p-6 shadow-sm h-full flex flex-col mb-4"
  >
    ...
    <div class="space-y-6 flex-1">
      ...
      <div class="group">
        ...
        ></textarea>
        <p v-else class="text-on-surface font-body-md italic">
      ...
      <!-- Notes -->
      <div class="group" v-if="store.isEditing || store.currentCharacterData.personality.notes">
        <label
          class="font-label-md text-label-md text-on-surface-variant mb-2 block uppercase tracking-wider group-focus-within:text-secondary-fixed transition-colors flex items-center justify-between"
        >
          Notes
          <span class="material-symbols-outlined text-xs opacity-50">notes</span>
        </label>
        <textarea
          v-if="store.isEditing"
          v-model="store.currentCharacterData.personality.notes"
          class="w-full bg-surface-dim border border-outline-variant rounded p-3 text-on-surface font-body-md focus:border-secondary-fixed focus:ring-1 focus:ring-secondary-fixed outline-none transition-all h-24"
          placeholder="Additional personality notes or roleplaying cues..."
        ></textarea>
        <p v-else class="text-on-surface font-body-md italic">
          {{ store.currentCharacterData.personality.notes }}
        </p>
      </div>
    </div>
  </section>
